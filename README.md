...
| subtract_from          | `"none"` or `"home"` or `"grid"` or `"both"` | `"none"` | Controls where this individual device is subtracted from: `none` (do not subtract), `home` (subtract from Home consumption), `grid` (subtract from Grid consumption for loads measured by the grid meter but not by the home meter), or `both` (subtract from both Home and Grid). |
...
| secondary_info      | `object`                                    | `undefined`          | Check [Secondary Info Object](#secondary-info-configuration)                                                                                                                                                                                                                                                |
| subtract_individual | `boolean`                                   | false                | Legacy option: if set to `true`, all individual devices are subtracted from the home consumption (overrides per-device `subtract_from`).                                                                                                                                                                    |
