...
| hide_if_lower_than     | `number`  | `undefined`                                    | If set, the device will be hidden until its power is at least this value. Use this for per-device thresholds such as `50` or `100` watts. Takes precedence over `display_zero_tolerance`. |
| display_zero_state     | `boolean` | `true`                                         | If set to `true`, the state will be shown even if it is `0`. If set to `false`, the state will be hidden if it is `0`.                                                               |
